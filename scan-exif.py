...
#            print tag
            jpgt = time.mktime(time.strptime( str(tag), "%Y:%m:%d %H:%M:%S" )) - dif
            l = 0
            u = len(pts)-1
            if pts[l]["tme"]<=jpgt and jpgt<=pts[u]["tme"]:
                ...
                #print "%s(%s) is between %s,%s(%s) and %s,%s(%s): %s,%s"%( ( \
                #    fn, jpgt, ) + tuple(pts[l].values()) + tuple(pts[u].values()) + ( lat, lon, ) )
                wpts.append( """<wpt lat="%f" lon="%f">
    <ele>%f</ele>
    <name>%s</name>
    <cmt>%s</cmt>
    <desc>%s</desc>
